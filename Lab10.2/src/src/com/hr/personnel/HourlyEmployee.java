...
import src.gov.irs.IllegalWageException;

import java.time.LocalDate;

import static src.gov.irs.TaxPayer.HOURLY_TAX_RATE;
...
public class HourlyEmployee extends Employee {
    ...
    public void setRate(double rate) throws IllegalWageException {
        if (rate < FEDERAL_MINIMUM_WAGE) {
            throw new IllegalWageException("Illegal wage " + rate + " does not meet federal minimum wage requirement");
        } else {
            this.rate = rate;
        }
    }
